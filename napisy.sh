...
cat /tmp/Wiadomosci.srt | grep -v 'STRONA 777' | grep -v 'NAPISY DLA NIESŁYSZĄCYCH' | grep -v 'WYKAZ PROGRAM' | grep -v ",,," | grep -v 'KOLEJNA EMISJA' | grep -v 'INFORMACJA O AUDYCJACH' | grep -v " / " > transcriptions_srt/Wiadomosci_$(date +%d.%m.%Y).srt

#!/bin/bash
/usr/local/bin/ccextractor "$1" -nofc -nots -tpage 778 -o Wiadomosci.srt
grep -v 'STRONA 777' Wiadomosci.srt | grep -v 'NAPISY DLA NIESŁYSZĄCYCH' | grep -v 'WYKAZ PROGRAM' | grep -v ",,," | grep -v 'KOLEJNA EMISJA' | grep -v " / " > transcriptions_srt/Wiadomosci_$(date +%d.%m.%Y).srt
